...
async function request(path2, query = {}) {
    ...
    const resp = await fetch(config.getItem("ext.ncm.apiEndpoint") + path2 + "?" + formatted, {
        headers
    });
...
async function fetchMetadata(...ids) {
    ...
    for (let idsArr of split) {
        pool.submit(async () => {
            const resp = await request("/song/detail", {
                ids: idsArr.join(",")
            });
...
ExtensionConfig.ncm = {
    ...
    player: {
        async getPlayUrl(path2, isDownload, count = 0) {
            ...
            const resp = await request("/song/url", {
                id, br: getBr()
            });
        ...
        async getLyrics(path2) {
            ...
            const resp = await request("/lyric", {
                id
            });
            cachedLyrics[id] = "";
    ...
    musicList: {
        async _import(callback, id, isUpdate = false) {
            ...
            try {
                const resp = await request("/playlist/detail", {
                    id
                });
